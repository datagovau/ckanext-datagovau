...
SPATIAL_INGESTOR_AUTHORIZATION = os.environ.get('SPATIAL_INGESTOR_AUTHORIZATION')
GEOSERVER_DATASTORE_URL= os.environ.get( 'GEOSERVER_DATASTORE_URL')
GEOSERVER_ADMIN_URL= os.environ.get( 'GEOSERVER_ADMIN_URL')
GEOSERVER_PUBLIC_URL= os.environ.get( 'CKAN_SITE_URL') + '/geoserver'

opener = urllib.request.build_opener()
opener.addheaders = [('Authorization', SPATIAL_INGESTOR_AUTHORIZATION)]
...
class IngestionException(Exception):
    ...
    def __str__(self):
        ...
        return "{0} ({1})".format(self.args[0], str(dataset.get('name')))
...
def _get_tmp_path():
    return "/tmp" #_get_dir_from_config('ckanext.datagovau.spatialingestor.tmp_dir')
...
def _get_db_settings():
    ...
    db_details_match = re.match(u''.join(regex), GEOSERVER_DATASTORE_URL)
    postgis_info = db_details_match.groupdict()
...
def _apply_sld(name, workspace, layer_name, url=None, filepath=None):
    ...
    if url:
        r = _make_request(
            requests.get,
            url, headers={'Authorization': SPATIAL_INGESTOR_AUTHORIZATION})
        if r and r.ok:
...
def _apply_sld_resources(sld_res, workspace, layer_name):
    ...
    r = _make_request(
        requests.get,
        sld_res['url'], headers={'Authorization': SPATIAL_INGESTOR_AUTHORIZATION})
